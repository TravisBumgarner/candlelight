...
func draw_queue(canvas, tile_id):
	var y_offset = Vector2i(0, 0)
	for piece in pieces_queue:
		for point in piece.preview.shape:
			canvas.set_cell(Consts.BACKGROUND_LAYER, consts.QUEUE_PREVIEW_ORIGIN + point + y_offset, tile_id, Consts.FOREGROUND_PIECE_COLOR) 
		y_offset += Vector2i(0, piece.preview.height + 1)

func erase_queue(canvas):
	utils.erase_area(canvas, Consts.QUEUE_PREVIEW_ORIGIN, Consts.QUEUE_PREVIEW_END, Consts.BACKGROUND_LAYER)
